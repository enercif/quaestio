...
<div
	class={cn(
		'border-input bg-background selection:bg-primary dark:bg-input/30 relative flex min-h-9 w-full flex-wrap place-items-center gap-1 rounded-md border py-0.5 pr-1 pl-1 disabled:opacity-50 aria-disabled:cursor-not-allowed',
		className
	)}
...
>
	...
	<input
		...
		aria-activedescendant={suggestionIndex !== undefined
			...
			: undefined}
		class="placeholder:text-muted-foreground min-w-32 shrink grow basis-0 border-none bg-transparent px-2 outline-hidden focus:outline-hidden disabled:cursor-not-allowed data-[invalid=true]:text-red-500 md:text-sm"
	/>
	{#if showSuggestions}
		<div
			bind:this={listboxEl}
